...
async function checkEmails() {
  const { data: properties, error } = await supabase
    .from('properties')
    .select('id, title, manager_name, manager_email, manager_id')
    .limit(20);
  ...
  console.log('Sample Properties with Email:');
  properties.forEach(p => {
    console.log(`ID: ${p.id}, Title: ${p.title}, Name: ${p.manager_name}, Email: ${p.manager_email}, ID: ${p.manager_id}`);
  });
}
